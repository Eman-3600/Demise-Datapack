...
execute if entity @s[scores={D_Tutorial=200}] run tellraw @s {"text":"Use /trigger ghost to change (will not work while banished).","color":"red"}
execute if entity @s[scores={D_Tutorial=260}] run tellraw @s {"text":"Manifesting weakens you for 3 seconds.","color":"red"}
execute if entity @s[scores={D_Tutorial=340}] run tellraw @s {"text":"Additionally, your material form burns in daylight.","color":"red"}
